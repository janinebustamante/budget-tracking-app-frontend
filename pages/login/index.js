        ...
        .then(data => {
            ...
            if (data.accessToken !== undefined) {
                ...
                if (data.error === 'google-auth-error') {
                    alert('Google Auth Error', 'Google authentication procedure failed.', 'error')
                    //3 arguments: title, text, icon(error/success)
                } else if (data.error === 'login-type-error') {
                    alert('Login Type Error', 'You may have registered through a different login procedure.', 'error')
                }
            }
        })
    };
